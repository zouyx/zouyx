### Hi there, I'm zouyx 👋🤖

> _"The best way to predict the future is to build it — especially now, in the AI era."_

---

#### 🧠 About Me

* 🤔 Apache [Dubbo](https://github.com/apache/dubbo) / [apolloconfig](https://github.com/apolloconfig/apollo) PMC.
* 😄 Concentrate on distributed services & **AI-native infrastructure**.
* 📫 joezou@apache.org || https://zouyx.github.io/about/
* 🚀 Exploring the intersection of **LLMs**, **cloud native**, and **distributed systems**.
* 🌱 Currently learning: **AI Agents**, **RAG pipelines**, and **LLM inference optimization**.
* 🤝 Open to collaborate on AI + open-source distributed computing projects.
* 🤔 Reject 996, WLB only.

---

#### 🔥 Tech Stack & Interests

![Go](https://img.shields.io/badge/-Go-00ADD8?style=flat-square&logo=go&logoColor=white)
![Java](https://img.shields.io/badge/-Java-007396?style=flat-square&logo=java&logoColor=white)
![Python](https://img.shields.io/badge/-Python-3776AB?style=flat-square&logo=python&logoColor=white)
![Kubernetes](https://img.shields.io/badge/-Kubernetes-326CE5?style=flat-square&logo=kubernetes&logoColor=white)
![LLM](https://img.shields.io/badge/-LLM%20%2F%20AI%20Agents-FF6F00?style=flat-square&logo=openai&logoColor=white)
![Apache](https://img.shields.io/badge/-Apache-D22128?style=flat-square&logo=apache&logoColor=white)

---

#### 🤖 AI Era Focus

| Area | What I'm Exploring |
|---|---|
| 🧩 LLM Integration | Building AI-augmented microservices |
| ⚙️ AI Infra | Inference serving, model routing, edge AI |
| 🔗 Distributed AI | Federated learning, AI-native service mesh |
| 🛠️ Dev Tools | AI-powered coding assistants & observability |

---

#### 📡 Find Me

![gongzhonghao](https://user-images.githubusercontent.com/3828072/112783769-a565ea00-9082-11eb-9fff-da60be251a87.png)
